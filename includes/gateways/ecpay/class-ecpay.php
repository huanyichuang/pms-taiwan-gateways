	...
	if ( class_exists( 'PMS_Payment_Gateway' ) ) {
		class PMS_Payment_Gateway_TW_ECPay extends PMS_Payment_Gateway {
			...
			public function process_sign_up() {
				...
				$payment-> update(
					array(
						'type' => apply_filters( 'pms_ecpay_payment_type', 'web_accept_ecpay', $this, $settings )
					)
				);
				...
				if( !empty( $settings['recurring'] ) ) {
					...
					$payment->log_data( 'ecpay_args', array(
						'data' => $ecpay_args,
					) );
				}
				...
				try {
					...
					$obj->Send['OrderResultURL']    = '' === $ecpay_args['result_url'] ? $ecpay_args['return_url'] : $ecpay_args['result_url'];
				...
				if ( 'completed' !== $payment->status && 0 !== $payment->amount ) {
				...
				if ( $verified ) {
					...
					if ( preg_match( '/^Credit.*/', $payment_data['type'] ) ) {
						...
						if ( 'completed' === $payment_data['status'] ) {
							...
							$member_subscriptions = pms_get_member_subscriptions(
								array(
							...
							foreach ( $member_subscriptions as $member_subscription ) {
								...
								if ( $member_subscription->status == 'pending' ) {
									pms_add_member_subscription_log(
			...
			public function validate_credentials() {
				...
				if ( false === pms_get_ecpay_merchant() )
					pms_errors()->add( 'form_general', __( 'The selected gateway is not configured correctly. Contact the system administrator.', 'pms-taiwan-gateways' ) );
